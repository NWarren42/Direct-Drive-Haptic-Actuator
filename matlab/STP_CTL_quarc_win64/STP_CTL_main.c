...
typedef RT_MODEL_STP_CTL_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * STP_CTL(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(STP_CTL_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://STP_CTL:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: STP_CTL -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
